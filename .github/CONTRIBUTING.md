...
Every push and pull request runs six jobs (`.github/workflows/ci.yaml`,
RFC-034 Handoff 02, RFC-035 Handoff 03). Reproduce each locally before
opening a PR:
...
| `msrv` | The crate builds at the declared MSRV (`rust-version` in `Cargo.toml`), not just a newer default toolchain | `rustup toolchain install 1.88.0 && rustup run 1.88.0 cargo check --all-features` |
