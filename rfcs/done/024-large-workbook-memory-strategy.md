...
**Status.** Partially implemented (2.0.0–2.2.3) — verified 2026-08-16; cancellation deferral closed 2026-08-17 (M7 Handoff 03). Cancellation is now polled once per sheet pair **and** at an interval inside each sheet's own read and compare phases (50,000 cells, derived from a measured ~1.9 µs/cell budget — see `docs/src/maintainers/performance.md`), so the acceptance criterion below ("can cancel between row chunks or cell batches") is met. Measurement (M7 Handoff 01) found the prior gap was worse than this line stated: on a single-sheet workbook — the ordinary shape of a spreadsheet — cancellation was not merely coarse-grained, it was never observed at all, since the once-per-sheet-pair check never fires a second time when there is no second sheet. **§7's density choice (`Sparse`/`Dense`) is declined, not deferred, as of 2026-08-17** — measured at +12.4% per populated cell (M7 Handoff 01), which does not justify a density heuristic and two code paths through the comparison loop; see `docs/src/maintainers/performance.md`. **§3's "avoid unnecessary duplication of cell data" goal is closed for the alignment clone as of 2026-08-17 (M7 Handoff 04)** — `cell_map_to_align` cloned every `CellValue` in both sheets into a second `BTreeMap` for every non-`Positional` comparison, measured at +33% of peak (M7 Handoff 01); alignment now borrows the same `CellMap` the compare loop already holds. **Corrected M9 unit 00 (2026-09-26):** this line said "the re-measured delta collapses to 0.0% at both scales tested". That re-measurement keyed `RowKey` on `columns: vec![0]`, and key columns are **1-based**, so no row had a key and the LCS ran on two empty sequences — a delta of zero for an alignment that did not happen. **The clone's deletion stands and the +33% is gone; "non-`Positional` alignment costs nothing" does not.** What replaced the clone is the LCS table, quadratic in rows, ≈100 MB at the default `max_alignment_product`. See Q2 in `docs/src/maintainers/performance.md`. Both `CellMap`s remaining resident during a comparison (§7's "for sparse sheets, prefer maps keyed by coordinate" holding two of these per pair) is unchanged by this and not isolable from calamine's own buffers by external measurement — see Q2 in `docs/src/maintainers/performance.md`.
