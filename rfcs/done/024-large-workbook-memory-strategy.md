...
**Status.** Partially implemented (2.0.0–2.2.3) — verified 2026-08-16; cancellation deferral closed 2026-08-17 (M7 Handoff 03). Cancellation is now polled once per sheet pair **and** at an interval inside each sheet's own read and compare phases (50,000 cells, derived from a measured ~1.9 µs/cell budget — see `docs/src/maintainers/performance.md`), so the acceptance criterion below ("can cancel between row chunks or cell batches") is met. Measurement (M7 Handoff 01) found the prior gap was worse than this line stated: on a single-sheet workbook — the ordinary shape of a spreadsheet — cancellation was not merely coarse-grained, it was never observed at all, since the once-per-sheet-pair check never fires a second time when there is no second sheet. **§7's density choice (`Sparse`/`Dense`) is declined, not deferred, as of 2026-08-17** — measured at +12.4% per populated cell (M7 Handoff 01), which does not justify a density heuristic and two code paths through the comparison loop; see `docs/src/maintainers/performance.md`.
