...
| 05 | **O-B — the corpus cannot reach the warnings M8 unit 02 exists to surface.** **Route decided 2026-09-27: port ForskScope's generator shapes into `examples/gen-fixtures.rs`**, attributed. **And apply RFC-036 §5.1's sharpened definition** (2026-09-27): each new matrix row states **which behaviour** its assertion protects — correctness or resource cost — because an assertion on one would not fail if the other broke. Row-count asymmetry is a **resource** dimension here (it is the only shape that reaches f131's ceiling) and a **correctness** one in a line diff, where ForskScope measured every asymmetric shape as *cheaper* than the symmetric pair of the same size. Their `edit_blank_key_row` and `insert_top_blank_keys` reach these warnings; `sort_by_name` adds the reordering case the corpus lacks entirely, where every alignment mode is worse than `Positional`. RFC-036 rejects a third-party workbook *corpus* because the gap is **structural patterns, which are generatable** — a generator is what it asks for, not what it forbids, and fixtures our tool emits satisfy its "generated reproducibly" criterion. The architect's earlier provenance boundary was stricter than the rule it cited.** Swept independently at review time: 19 scenarios × 4 alignment modes = **76 runs, zero sheet-level warnings**. No fixture trips `AlignmentBoundExceeded` or `DuplicateAlignmentKey`, and RFC-036's coverage matrix has no row for either. **This is D1's shape again** — a corpus that cannot reach the code it exists to guard — and it should be worked beside D1, not apart from it. Found by the implementer 2026-09-25. |
